...
class JogoDAO
{
    ...
    public function save($datajogo, $timecasa, $timevisitante, $imagem): int
    {
        try {
            $query = "INSERT INTO jogos (timecasa, timevisitante, datajogo, imagem) 
                VALUES (:timecasa, :timevisitante, :datajogo, :imagem);";

            $stmt = $this->dbh->prepare($query);
            $stmt->bindValue(':timecasa', $timecasa);
            $stmt->bindValue(':timevisitante', $timevisitante);
            $stmt->bindValue(':datajogo', $datajogo);
